...
namespace IT
{
	std::unordered_map<std::string, bool> uniqueLiterals;
			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(17) << "Тип данных:"<< std::setw(15) << "Значение:" << std::setw(27) << "Длина строки:" << std::setw(20) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::L)
				{
					...
					{
					case 1:
						literalValue = std::to_string(this->table[i].value.vint);
						break;
					case 2:
						literalValue = this->table[i].value.vstr.str;
						break;
					}

					if (!uniqueLiterals[literalValue])
					{
						uniqueLiterals[literalValue] = true;
						*(idStream) << "   " << this->table[i].id << "\t\t\t";
						switch (this->table[i].iddatatype)
						{
						case 1:
